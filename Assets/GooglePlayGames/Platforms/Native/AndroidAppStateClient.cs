...
internal class AndroidAppStateClient : AppStateClient {
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        OurUtils.Logger.d("LoadState, slot=" + slot);
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        OurUtils.Logger.d("UpdateState, slot=" + slot);
        using (var apiClient = GetApiClient(mServices)) {
    ...
    private class OnStateResultProxy : AndroidJavaProxy {
        ...
        private void OnStateConflict(int stateKey, string resolvedVersion,
            ...
            OurUtils.Logger.d("OnStateResultProxy.onStateConflict called, stateKey=" + stateKey +
            ...
            if (mListener != null) {
                OurUtils.Logger.d("OnStateResultProxy.onStateConflict invoking conflict callback.");
                ...
                OurUtils.Logger.w("No conflict callback specified! Cannot resolve cloud save conflict.");
            }
        }

        private void ResolveState(int slot, string resolvedVersion, byte[] resolvedData,
            OnStateLoadedListener listener) {
            OurUtils.Logger.d(string.Format("AndroidClient.ResolveState, slot={0}, ver={1}, " +
        ...
        private void OnStateLoaded(int statusCode, int stateKey, byte[] localData) {
            OurUtils.Logger.d("OnStateResultProxy.onStateLoaded called, status " + statusCode +
                ", stateKey=" + stateKey);
            debugLogData("localData", localData);
            ...
            switch (statusCode) {
                case STATUS_OK:
                    OurUtils.Logger.d("Status is OK, so success.");
                    success = true;
                    break;
                case STATUS_NO_DATA:
                    OurUtils.Logger.d("Status is NO DATA (no network?), so it's a failure.");
                    success = false;
                    localData = null;
                    break;
                case STATUS_STALE_DATA:
                    OurUtils.Logger.d("Status is STALE DATA, so considering as success.");
                    success = true;
                    break;
                case STATUS_KEY_NOT_FOUND:
                    OurUtils.Logger.d("Status is KEY NOT FOUND, which is a success, but with no data.");
                    success = true;
                    localData = null;
                    break;
                default:
                    OurUtils.Logger.e("Cloud load failed with status code " + statusCode);
                    success = false;
                    localData = null;
                    break;
            }
            ...
            if (mListener != null) {
                OurUtils.Logger.d("OnStateResultProxy.onStateLoaded invoking load callback.");
                PlayGamesHelperObject.RunOnGameThread(() => {
                    mListener.OnStateLoaded(success, stateKey, localData);
                });
            } else {
                OurUtils.Logger.w("No load callback specified!");
            }
        }

        private void debugLogData(string tag, byte[] data) {
				OurUtils.Logger.d("   " + tag + ": " + OurUtils.Logger.describe(data));
        }

        public void onResult(AndroidJavaObject result) {
            OurUtils.Logger.d("OnStateResultProxy.onResult, result=" + result);

            int statusCode = GetStatusCode(result);
            OurUtils.Logger.d("OnStateResultProxy: status code is " + statusCode);

            if (result == null) {
                OurUtils.Logger.e("OnStateResultProxy: result is null.");
                return;
            }

            OurUtils.Logger.d("OnstateResultProxy: retrieving result objects...");
            ...
            OurUtils.Logger.d("Got result objects.");
            OurUtils.Logger.d("loadedResult = " + loadedResult);
            OurUtils.Logger.d("conflictResult = " + conflictResult);

            if (conflictResult != null) {
                OurUtils.Logger.d("OnStateResultProxy: processing conflict.");
                ...
                OurUtils.Logger.d("OnStateResultProxy: conflict args parsed, calling.");
                OnStateConflict(stateKey, ver, localData, serverData);
            } else if (loadedResult != null) {
                OurUtils.Logger.d("OnStateResultProxy: processing normal load.");
                int stateKey = loadedResult.Call<int>("getStateKey");
                byte[] localData = ToByteArray(loadedResult.NullSafeCall("getLocalData"));
                OurUtils.Logger.d("OnStateResultProxy: loaded args parsed, calling.");
                OnStateLoaded(statusCode, stateKey, localData);
            } else {
                OurUtils.Logger.e("OnStateResultProxy: both loadedResult and conflictResult are null!");
            }
        }
    }
